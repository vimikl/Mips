...
LOCAL const UChar hysterese_count = 5;
...
__interrupt Void TIMER1_A1_ISR(Void) {
   ...
   //btn1
    ...
    if(TSTBIT(P1IN, BIT0) && count1 < 5) {
      count1++;
    } else if(count1 > 0) {
      count1--;
    }
    if (!state1) {
      if (count1 == hysterese_count) {
         state1=1;
         // send button event
    ...
    if(TSTBIT(P1IN, BIT1) && count2 < 5) {
      count2++;
    } else if(count2 > 0) {
      count2--;
    }
    if (!state2) {
      if (count2 == hysterese_count) {
         state2=1;
         // send button event
