...
LOCAL const UChar muster_size[6] = {
   10,
   6,
   2,
   12,
...
LOCAL UChar muster_idx;

LOCAL ULong *blink_muster;
LOCAL UChar *blink_muster_size;

LOCAL UChar new_blink_muster;
LOCAL UChar new_blink_muster_size;
...
GLOBAL Void TA0_init(Void) {
   ...
   blink_muster_size = (UChar *) &muster_size[0];
...
__interrupt Void TIMER0_A1_ISR(Void) {
   ...
   if (--muster_idx < 0) {
      if (switch_muster) {
          ...
          blink_muster_size = (UChar *) &muster_size[new_blink_muster];
          switch_muster = 0;
      }
      muster_idx = blink_muster_size;
   }
